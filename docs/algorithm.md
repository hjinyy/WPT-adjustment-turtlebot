...
## 7. 라인 트레이서와 칼만 필터 보조 정합

현재 정합 목표는 "라인을 잘 따라가는 것"이 아니라 **목표 AprilTag pair의 두 마커 사이 중점에 TurtleBot 수신 코일을 정확히 정지시키는 것**입니다. 따라서 최종 정지 조건은 계속 AprilTag pair의 midpoint/angle 오차와 `stable_frames_required`가 담당합니다.

전방 카메라는 z축 위에서 내려다보는 top-down 관측으로 가정합니다. 화면 상단이 진행 방향이며, 검정 전기테이프 라인은 `line_tracking.camera`(기본 `front`)에서 검출합니다. 라인 관측은 `LineDetector`가 `x_error`, `angle_error_deg`, `confidence`로 변환하고, `ErrorKalmanFilter`가 프레임 간 흔들림을 완화합니다.

제어 우선순위는 다음과 같습니다.

1. 목표 AprilTag pair가 보이면 pair midpoint/angle 기반 정밀 정합 명령을 주 명령으로 사용합니다. 라인은 `align_line_weight`만큼 각속도 보조항으로만 섞습니다.
2. pair가 아직 안 보이고 라인이 보이면 `SEARCH_COIL`에서 저속으로 라인을 따라갑니다.
3. pair도 라인도 안 보이면 기존 직진 burst/정지 확인 탐색으로 돌아갑니다. 라인이 없다고 정지 실패로 보지 않습니다.
4. `FINAL_STOP`은 원시 AprilTag pair 오차가 기준 안에 N프레임 연속 들어왔을 때만 진입합니다.

ROS 없이 라인 검출 상태를 확인하려면 다음처럼 실행합니다.
...
최종 목표는 라인을 따라가는 것이 아니라 목표 AprilTag pair의 두 마커 사이 midpoint에 TurtleBot 코일을 멈추는 것입니다. 기존의 대칭 허용 오차는 목표보다 조금 앞/뒤 모두를 같은 정합으로 보므로, 로봇이 목표를 지나친 뒤 뒤로 보정하는 왕복 미세 조정이 생길 수 있습니다.

`alignment.undershoot_stop`은 이 문제를 줄이기 위해 `ALIGN_COIL`의 최종 정지 판정을 목표 직전 쪽으로 편향합니다. 기본값에서는 `approach_y_sign: -1`이므로 negative `y_error`가 아직 목표를 지나치지 않은 접근 방향이고, positive `y_error`는 overshoot로 간주합니다. 따라서 `undershoot_y_px` 범위 안의 목표 직전 오차는 허용하지만, `overshoot_y_px`는 아주 작게 둡니다.

`block_reverse_linear: true`이면 `ALIGN_COIL` 중 후진 선속도 명령을 0으로 막습니다. `block_pair_loss_linear_backoff: true`이면 pair를 순간적으로 잃었을 때도 선형 후진 back-off는 막고 각속도 되돌림만 허용합니다. 즉 최종 접근은 앞으로 조금씩 접근하다가 목표 직전에 멈추는 쪽이며, 앞으로 갔다가 뒤로 갔다가 하는 보정 루프를 기본 전략으로 쓰지 않습니다. 실제 카메라 축이 반대로 잡혔다면 `approach_y_sign`과 `forward_linear_sign`을 함께 바꿔야 합니다.