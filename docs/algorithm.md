...
`four_coil_map`/`station_map`처럼 목표 코일/스테이션이 명확히 지정된 경우, 헤드 태그 탐색 없이 바로 `ALIGN_COIL`에서 시작합니다(`_starts_at_alignment()`). 레거시 shelf 모드에서만 `SEARCH_HEAD_TAG`부터 시작합니다.
...
| `ENTER_SHELF` | 목표 페어(예: west+east) 인식 여부 | 둘 다 안 보이면 저속 전진, 한쪽만 보이면 정지 대기, 둘 다 보이면 `ALIGN_COIL`로 전환 |
