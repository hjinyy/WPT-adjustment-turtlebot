...
6. 2x2 코일 스테이지 물리 배치 + 오픈루프(dead-reckoning) 선반 간 이동 스크립트 (`scripts/drive_to_shelf.py`)
7. 순수 Python 단위 테스트
8. 충전 관제 서버(tserver.local:8000) 연동 에이전트 (`scripts/server_agent.py`)
9. (선택) ROS2 패키지로 빌드해서 쓰는 정밀 pair 정합 노드 (`wpt_alignment_node`)
